...
public class KindEditorController {
  ...
  private ArticleMapperService articleMapper;
    ...
    /**
     *  添加文章并初始化访问量等
     * @param article  前台传过来的文章
     * @return    跳转页面
     */
    @RequestMapping("/release")
    public String release(HttpServletRequest request,Article article, Model model,MultipartFile imgFile1)throws Exception{

        if(imgFile1!=null&&imgFile1.getOriginalFilename()!=null&&!imgFile1.getOriginalFilename().trim().equals(""))
        {
            String img=imgFile1.getOriginalFilename();
            ServletContext application = request.getSession().getServletContext();
            String savePath = application.getRealPath("/") + "attached/image/";
...
//            return getError("上传目录不存在。");
            ...
            //新的封面图片名字:
            String nweImgName=UUID.randomUUID()+img.substring( img.lastIndexOf("."));
            File newFile=new File(savePath+nweImgName);
            imgFile1.transferTo(newFile);
            System.out.println(newFile.getAbsolutePath());
            article.setCoverimage("<img   src='"+"/attached/image/"+ymd+"/"+nweImgName+"' />");
    ...
    public String helloHtml(Map<String,Object> map)throws Exception{
  
       map.put("hello","from TemplateController.helloHtml");  
       return"hello";
    }  
    @RequestMapping(value = "/admin/kindeditor/file-upload.do", method = RequestMethod.POST)  
    @ResponseBody  
...
//            FileItem item = (FileItem) itr.next();  
    ...
    @RequestMapping(value = "/admin/kindeditor/file-manager.do")  
